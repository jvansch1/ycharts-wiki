...
* [South](http://south.aeracode.org/docs/tutorial/index.html) handles database migrations for Django.
...
* [Whammy.js](https://github.com/ycharts/ycharts/blob/develop/apps/main/static/main/js/base/base.js) is our in house library we use to create complete JS apps like our [Stock Screener](http://ycharts.com/stock_screener), or our [stock charts](http://ycharts.com/companies/AAPL/chart). It is built on top of JQuery/Sammy.js/Mustache.js and provides a way to reuse widgets (like our 3-tiered metric dropdown) on our site in many different contexts with a minimum of code duplication. DEPRECATED.
* [ychart_fund.js](https://github.com/ycharts/ycharts/blob/develop/apps/charts/static/charts/js/base/ychart_fund.js) sits on top of Flot and acts as the go between between our data and Flot. It handles specific methods for creating good financial charts like data formatting, latest value labeling, pecularities with current day intraday charting, etc.
* [ychart_tech.js](https://github.com/ycharts/ycharts/blob/develop/apps/charts/static/charts/js/base/ychart_tech.js) sits on top of Flot and acts as the go between between our technical data and Flot. It knows how to calculate and draw all the pecularities of technical charts.
...
* **Admin** one machine per environment runs periodic scripts using cron and also runs various continuously running scripts that update stock quotes, import economic data and provide other functions that need to happen 24/7.
...
* **Redis** one machine per environment run our Redis DB server which powers our autocompleter and queue data among other things. If need be for scale, various individual services on this machine can easily be segregated to different machines.
* **Queue** one or machines per environment execute queued tasks, primarily financial calculations based on data that can be changed or updated throughout the day. Queue machines must be provisioned for peak loads as during earnings season the work they have to do is greater and we don't ever want tasks in the queue to sit for longer than a few hours.

### Deployment
For a full overview of our deployment setup, check out the [deployment prodedures doc](https://github.com/ycharts/ycharts/blob/master/docs/developer_notes/deployment_procedures.md) but here are a few quick tips:

1. Releasing to staging is no big deal.  Learn there.
2. Releasing to production is a big deal.  Don't do it unless you've had somebody walk you through the process a couple times.
...
The initial setup of a working development environment can be tricky.  Talk to any developers with questions, but start with our [dev setup docs](https://github.com/ycharts/ycharts/blob/develop/docs/developer_notes/dev_setup.md).
...
### Code Editors
Although you are free to use any editor you prefer, our development has standardized around SublimeText and Vim. We have helpful pointers about plugins to install and shortcuts to know for each. The SublimeText notes are [here](https://github.com/ycharts/ycharts/blob/develop/docs/developer_notes/sublime_getting_started.md) and the Vim notes are [here](https://github.com/ycharts/ycharts/blob/develop/docs/developer_notes/vim_notes.md)
...
2) For testing the YCharts Node codebase we use mocha.
...
* **tasks.py** - contains any tasks associated with a given app that can run asynchronously through a [celery task](http://celery
...
* **/migrations** - hold South migrations
...
* **admin** - contains urls.py defining all our various custom admin views.
...
* **chart_creator** - everything we need to power our [Chart Creator](http://ycharts.com/chart_creator)
...
* **industries** -  models and views for industries, and tasks that run on them.
...
* **sectors** - models and views for sectors, and tasks that run on them.
...
### /binaries
Stores some Java binaries we need. Basically for minification, compression of JS/CSS.
...
#### /docs Subdirectories
* **/aws** has docs for setting up machines on AWS
* **/deployment_notes** is where each new release's must-do tasks are kept
* **/developer_notes** is where most our code's documentation lives

Other folders have names that describe what is in them.  They are worth browsing briefly.

### /fabfile
This folder contains the modules that assist with deploying to staging and production.  They run using Fabric.

Again, check our [deployment prodedures doc](https://github.com/ycharts/ycharts/blob/master/docs/developer_notes/deployment_procedures.md) for details on this.

### /settings
Contains our Django settings files. Note that we split settings into develop, staging and production, because database and other settings differ depending on environments. active.py imports the currently active environment. It is by default set to development as is changed to production/staging as part of the deploy process.
...
### /templates
Contains all of our Django HTML templates.  The structure mirrors our apps almost perfectly.

### Top level files
* **menu.py** defines a custom menu in the Django Admin as specificed by django-admin-tools
* **urls.py** Is the site's main URL mapping file
...
The following commands should always be running on our admin box:
...
2) "Latest" tasks. These are tasks that are run regularly to calculate the latest value of a calculation. Since they are typically run once a day, it is important that they are never buried by a backlog of base tasks so they live in their own queue.
3) "Alert" tasks. These are tasks that trigger alerts in our alerts system. They have their own queue because we want to trigger alerts as soon as possible from the occurence of the triggering event.

To run locally:

    celery -A ycharts worker -E -Q main  # Runs all base tasks
    celery -A ycharts worker -E -Q alerts  # Runs all alert tasks
    celery -A ycharts worker -E -Q latestcalcs  # Runs all latest tasks
    celery -A ycharts worker -E -Q main,alerts,latestcalcs -l info # Runs both with logging output to the terminal

### Other scripts that should be continuously running
...
Where `<scriptname>` is one of the following
    ...
    indicators_data_import_one
    indicators_data_import_two
    indicators_data_import_three
    indicators_data_import_four
    news_populate_company_news_single
    news_populate_company_news_full
    news_populate_index_news
    news_populate_company_tweets

To check to see which scripts using Fabric:

On production

    fab -i ~/.ssh/ycharts-2014-01.pem production admin.monitor

On staging

    fab -i ~/.ssh/ycharts-2014-01.pem staging admin.monitor

    $ ssh ycharts_staging # login with ssh

Before starting all scripts one-by-one, if none are running, you can start all of them with:

On production

    fab -i ~/.ssh/ycharts-2014-01.pem production admin.start # Starts all scripts
    fab -i ~/.ssh/ycharts-2014-01.pem production admin.stop # Stops all scripts

On staging

    fab -i ~/.ssh/ycharts-2014-01.pem staging admin.start # Starts all scripts
    fab -i ~/.ssh/ycharts-2014-01.pem staging admin.stop # Stops all scripts
